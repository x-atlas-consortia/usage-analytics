...
####################################################################################################
# Globus HTTP access log parsing for file download events
#
# Intended crontab entry (adjust day-of-week and time as needed):
#   /hive/users/hive/scripts/PittCronJobs/analytics/usage-analytics/log-processing/globus-downloads-to-JSON/src/globus_access_log_extract.sh >> .../exec_info/bash_output.log 2>&1
#
...
# Logs Python output to PROCESS_DIR/exec_info/globus_access_python_output.log.
#
...
echo executing "python3 globus_access_log_extract.py with PYTHONPATH=$PYTHONPATH"
python3 "$SRC_DIR/globus_access_log_extract.py" --process-dir "$PROCESS_DIR" > "$EXEC_INFO_DIR/globus_access_python_output.log" 2>&1
